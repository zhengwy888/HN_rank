  ...
  inpdir=../hn_2kb2 # <= change this to where rcv1 data is. 
  ...
  log_fn=log_output/hn_2kb2_seq_${current_time}.log
  perf_fn=perf/hn_2kb2_seq-perf_${current_time}.csv
  echo 
  ...
  # 1000 -> crashed
  # TODO 600
  nodes=600 # number of neurons (weight vectors) in the convolution layers
  ../bin/cnet3pub $gpu cnn \
         nodes=$nodes resnorm_width=$nodes \
         data_dir=data trnname=hn_train-p${pch_sz} tstname=hn_test-p${pch_sz} \
